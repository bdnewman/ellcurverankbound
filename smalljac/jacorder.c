...
/*
	...
	The details are described in [SutherlandThesis] and [KedlayaSutherland2007].
	
	This code is still subject to occasional tweaking, so some debugging code has been
	left in the source (but commented out).
	
	6/1/2008: As of version 3.0, genus 1 order and structure computations are now handled in ecurve.c,
...
/*
    ...
    if a1 != JAC_INVALID_A1, it specifies the known value of a1.  if d is nonzero it specifies a known divisor of the group exponent

    The return value is 0 for an error, otherwise the number of multiples of the order of the largest subgroup found 
...
int jac_order (unsigned long *pP1, unsigned long Min, unsigned long Max, long a1, long d, int fExponentOnly, int *constraints, hc_poly c[1])
{
	...
	double x, z;
	long t;
	int full, ambiguous_exponent,tor3;
...
//printf ("%lu: M = %ld, W = %ld, a1 = %ld, Min = %ld, Max = %ld\n", p, M, W, a1, Min, Max);
	...
	if ( ! _ff_p1mod3 ) {													// for now only use 3tor when p=2mod3 and it is fast and easy
		tor3 = ecurve_3tor(c[0].f);
		if ( tor3 !=3 && tor3 != 9 ) tor3=1;									// don't deal with special cases
	}
#endif
#if HECURVE_GENUS == 2
	two_rank = hc_poly_two_rank (c);
	// computing 3tor takes 300-500 microseconds, don't use if  p is small or we already know a1.  Our 3tor method only works for depressed quintics!
	if ( a1 == JAC_INVALID_A1 && _ff_p > SMALLJAC_3TOR_P && c[0].d==5 && _ff_zero(c[0].f[4])  ) tor3 = hecurve_g2_3tor(c[0].f);
#endif
#if HECURVE_GENUS > 2
	if ( !(c[0].d&1) ) { err_printf ("Curve degree must be odd in genus > 2 in jac_order\n"); exit (0); }
	two_rank = ff_poly_count_factors(c[0].f, c[0].d) -1;
#endif
	e = ( two_rank >0 ? 2 : 1);
	if ( tor3 ) e*=tor3;
	if ( d ) e = ui_lcm(d,e);
	order = 0;
//printf ("Beginning jac_order loop with p=%ld, e=%ld, two_rank=%d for curve: ", _ff_p, e, two_rank);  ff_poly_print (c->f, c->d);
	for(;;) {
		M2 = M/e;
		...
		if ( ! o ) { err_printf ("%lu: search failed with e=%lu, M=%lu, W=%lu, m=%lu for element: ", p, e, M, W, SMALLJAC_M);  _jac_print(g);  return 0; }
		e *= o;
		...
		if ( ! two_rank && ! (order&0x1) ) { order += e2; assert (order&0x1); }		// if 2-rank is 0, order must be odd
		if ( order > Max ) { err_printf ("%7lu: No multiple of exponent %lu in interval (%lu,%lu) with 2-rank %d after computing order %lu with full = %d for element:   ", p, e, Min, Max, two_rank, o, full);  _jac_print(g);  ff_poly_print(c->f, c->d); puts (""); return 0; }
	...
	if ( ambiguous_exponent ) {
		...
		dbg_printf ("%9lu: Ambiguous exponent %lu, %lu possible orders in [%ld,%ld]\n", p, e2, ui_multiples_in_range (e2, Min, Max), Min, Max);
		...
		for ( i = 0 ; i < k ; i++ ) {
			...
			if ( q[i]*e2 <= Max ) {
				...
				if ( q_rank < 0 ) {
					info_printf ("%lu: Abandoned large %lu-Sylow subgroup computation, m= %lu,  SylowLimit = %lu.\n", p, q[i], m, SylowLimit);
					if ( abandoned_q ) { err_printf ("%lu: Abandoned second %lu-Sylow subgroup computation - this should be impossible Max = %lu, m = %lu, SylowLimit = %lu\n", p, q[i], Max, m, SylowLimit);  return 0; }
					abandoned_q = q[i];
				}
				for ( j = 0 ; j < q_rank ; j++ ) o *= ords[j];
				if ( o > e2 ) {
					dbg_printf ("%7lu: %lu-Sylow computation enlarged subgroup to %lu\n", p, q[i], o);
					e2 = o;
				} else {
					dbg_printf ("%7lu: %lu-Sylow computation failed to enlarge subgroup\n", p, q[i]);
				}
				order = _ui_ceil_ratio(Min,e2)*e2;
				if ( ! two_rank && ! (order&0x1) ) order += e2;		// if 2-rank is 0, order must be odd
				if ( order > Max ) { err_printf ("%lu: No multiple of subgroup order %lu in interval (%lu,%lu) after %lu-Sylow computation, 2-rank = %d\n", p, e2, Min, Max, q[i], two_rank); return 0; }
				if ( order + e2 > Max ) break;
				if ( ! two_rank && !((order+e2)&0x1) && order + 2*e2 > Max ) break;
				order = 0;
			}
		...
		if ( i == k ) { 
			...
			info_printf ("%lu: Ambiguity resolved by computing all but 1 Sylow subgroup using %lu group operations\n", p, jac_gops-n);
#endif
		}
		info_printf ("%lu: Ambiguity resolved using %lu group operations\n", p, jac_gops-n);
	}
	*pP1 = order;
	return 1;
}
...
*/
unsigned long jac_parallel_search (jac_t a[1],  unsigned long M, unsigned long W, unsigned long Min, unsigned long Max, int full, int parity, hc_poly c[1])
{
...
//printf ("Begin search p = %lu, M = %lu, W = %lu, Min = %lu, Max = %lu, parity = %d, full = %d\n", _ff_p, M, W, Min, Max, parity, full);  _jac_print(a[0]);
	if ( W == 0 ) { err_printf ("%9lu: W == 0 in smalljac_parallel_search, increasing to 1\n", _ff_p);  W = 1; }
...
//printf ("%lu: Beginning baby steps\n", _ff_p);
...
//printf ("%lu: baby steps used %lu gops\n", _ff_p, jac_gops-n);
...
//printf ("%d\n", matches[k]);
						if ( stash ) {
							...
							jac_exp_ui_powers (&s1, b, matches[k], c); 			// reconstruct baby for comparison
							sign = _jac_cmp (s1, giants[i]);
						}
						if ( sign ) {
							if ( o ) o1 = o;
							o = ( sign > 0 ? uvalue - matches[k] : uvalue + matches[k] );
//printf ("%9lu: Found a match at uvalue = %lu, k = %d of %d, matches[k] = %u, sign = %d, o=%ld, %ld gops\n", _ff_p, uvalue, k, cnt, matches[k], sign, o, jac_gops-n);
...
//printf ("%9lu: Found a match at dvalue = %lu, k = %d of %d, matches[k] = %u, sign = %d, o = %ld %ld gops\n", _ff_p, dvalue, k, cnt, matches[k], sign, o, jac_gops-n);
							if ( ! o ) { o = o1;  continue;	}						// can happen for small groups
							if ( o < 0 ) o = -o;									// can happen for small groups
							if ( ! full ) goto found;
							if ( o == o1 ) continue;								// yes this can happen - up and down steps can overlap in odd parity case
...
//printf ("giant steps in full computation, used %lu gops\n", jac_gops-n);	
	if ( ! o ) {
		err_printf ("%lu: No exponent found in smalljac_parallel_search, full=%d, parity=%d, baby_span = %d, GS = %ld, dvalue = %ld, uvalue = %ld, Min = %lu, Max = %lu!\n",
				 _ff_p, full, parity, baby_span, GS, dvalue, uvalue, Min, Max);  hc_poly_print(c);
		_jac_print(a[0]);
		return 0;
	}
...
*/
	jac_factored_order_ui ((unsigned long*)&o, a, p, h, w, c);		// we assume order fits in 63 bits
	jac_fastorder_gops += jac_gops-n;
//jac_exp_ui (&s1, a, o, c);
//if ( ! _jac_is_identity(s1) ) { err_printf ("%ld: %ld is not an exponent of ", _ff_p, o);  _jac_print(s1);  abort();}
//printf ("%lu: %lu is the order of ", _ff_p, o); _jac_print(a[0]);
...
// If repeats is nonzero, it specifies the number of calls the caller plans to make using the same base point
// All calls after the first call should have new_a=0 (but keep repeats the same)
int jac_search (mpz_t e[2], jac_t new_a[1],  unsigned long m, mpz_t Min, mpz_t Max, int repeats, hc_poly c[1])
{
	static int init;
	static mpz_t o, o1, o2, G;
	static jac_t a[1], b[32];
	jac_t baby, giant;
	jac_t s1, step, babystep;
	unsigned long S, tabbits, s, gs; 
...
//gmp_printf ("Min=%Zd, Max=%Zd, range = %Zd, m=%ld\n", Min, Max, G, m);
	mpz_fdiv_q_ui (o, G, 2*m);
	assert ( mpz_fits_ulong_p (o) );
	S = mpz_get_ui(o) + 1;
//printf ("range steps = %lu\n", S);
	s = (unsigned long) ceil(sqrt(repeats)*sqrt(S));
//printf ("baby steps  = %lu\n", s);
	assert ( log2(s)+log2(m)+1 < 64 );
	S = 2*s*m;										// giant step size, twice the span of the babies- should fit in 64 bits
//printf ("giant step size = %lu\n", S);
	mpz_fdiv_q_ui (o, G, S);
	assert ( mpz_fits_ulong_p (o) );
	gs = mpz_get_ui(o) + 1;
...
//gmp_out_printf ("Small search, search p = %d, m = %d, Min = %Zd, Max = %Zd, s = %d\n", _ff_p, m, Min, Max, s);  _jac_print(a[0]);	
	if ( new_a ) _jac_set (a[0], new_a[0]);
	if ( new_a ) {
	...
	// Compute first giant
	mpz_set(G, Min);
	mpz_add_ui (G, G, s*m);												// First giant starts one babyspan into the interval
		...
		if ( (cnt = smalljac_table_lookup (matches, &giant)) > 0 ) {
			for ( k = 0 ; k < cnt ; k++ ) {
				if ( matches[k] ) {
					...
					jac_exp_ui_powers (&s1, b, matches[k], c);
					if ( (sign = _jac_cmp(s1,giant)) ) {
						mpz_set(o, G);
						mpz_add_ui (o, o, j*S);
						if ( sign > 0 ) { mpz_sub_ui (o, o, (unsigned long)matches[k]*m); } else { mpz_add_ui (o, o, (unsigned long)matches[k]*m); }
					} else {
						mpz_set_ui (o, 0);
					}
				} else {
