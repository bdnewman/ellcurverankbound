...
int jac_vector_logarithm (unsigned long e[], jac_t a[], unsigned long ords[], int k, jac_t b[1], hc_poly c[1]);
...
*/
int jac_structure (long m[], hc_poly c[1], unsigned long N, int fExponent)
{
	...
	for ( i = 0 ; i < w ; i++ ) {
		...
		if ( q && M != q ) {
			...
			printf ("p=%ld: ", _ff_p);  hc_poly_print (c);
			exit (0);
		}
	...
	// sanity checks - can be removed eventually (but probably better to leave them in, they take no time)
...
/*
    ...
    subgroup that is at least as large as M but may not be the full p-Sylow subgroup.    It is assumed that the order of the p-Sylow subgroup fits in an unsigned long.
...
int jac_sylow (jac_t a[JAC_MAX_GENERATORS], unsigned long ords[JAC_MAX_GENERATORS], unsigned long p, unsigned long E, unsigned long M, unsigned long limit, hc_poly c[1])
{
	...
	unsigned long m, q, x, temp;
	int k, n, cnt, retries, min, d;
	int sts;

	for ( h = 0 ; ! (E%p) ; h++ ) E/= p;
	if ( ! h ) { err_printf ("Trivial call to smalljac_sylow E = %lu, p = %lu\n", E, p);  return 0; }
	
	dbg_printf ("%ld: jac_sylow computing %ld-sylow subgroup with h=%ld, reduced E=%ld, M=%ld, limit=%ld\n", _ff_p, p, h, E, M, limit);
//printf ("Curve over F_%ld is: ", _ff_p); ff_poly_print (c[0].f, 6);
	k = 0;
	...
	if ( ords[k] < 2 ) { err_printf ("%ld: smalljac_sylow couldn't find any generators for %lu-Sylow subgroup - bad exponent %ld?!\n", _ff_p, p, E);  exit (0); }
	dbg_printf ("First generator for %lu-sylow subgroup has order %lu, E = %lu, M = %lu, limit = %lu\n", p, ords[k], E, M, limit);
	if ( dbg_level ) _jac_print (b);
	k++;
	// adjust retries based on p to achieve probabilty > 2^confidence
	retries = ceil(CONFIDENCE / log2(p));
	for ( cnt = 0 ; cnt < retries ; ) {
		for ( n = 0, x = 1 ; n < k ; x *= ords[n], n++ );
		// if we have exceeded the limit, return an error
		if ( limit && x > limit ){ dbg_printf ("Sylow subgroup computation terminated unsuccessfully due to %ld > %ld\n", x, limit);  return -1; }
		// if increasing the size of the p-group we have found would make the group size > M then we must be done
		if ( M && x > M/p ) { dbg_printf ("Sylow subgroup computation successfully terminated due to bound M = %ld\n", M);  break; }
		// note that we keep searching even if we have 2g generators, because we could need to extend the order of one of them, but k should never exceed 2g+1
		assert ( k <= 2*c->g );
		_jac_random (a[k], c[0]);
		jac_exp_ui (a+k, a+k, E, c);
repeat:
		...
		if ( limit && x > limit/p ){ dbg_printf ("Sylow subgroup computation terminated unsuccessfully due to %ld*%ld > %ld\n", p, x, limit);  return -1; }
		ords[k] = jac_pp_order_ui (&b, p, h, c);
		if ( ords[k] < 2 ) { err_printf ("unexpected order %lu returned from jac_pp_order_ui", ords[k]);  exit (0); }
		dbg_printf ("Found unspanned element with order %lu\n", ords[k]);
		if ( dbg_level ) _jac_print (b);
		...
		if ( m < ords[k] ) {
			...
			if ( min == -1 ) {
				for ( n = 0 ; n < k ; n++ ) {
					if ( e[n] != 0 ) {
						if ( (e[n]%m) != 0 ) { out_printf ("Problem, %lu not divisible by %lu\n", e[n], m);  exit (1); }
				...
				if ( temp != ords[k] ) { err_printf ("%lu: Order mismatch 1, %lu != %lu for element ", _ff_p, temp, ords[k]);  _jac_print(a[k]);  exit (0); }
				dbg_printf ("Added adjusted generator %d with order %lu using relation min == -1.\n", k, ords[k]);
				assert (k < 2*c->g);
				k++;
			} else {
				if ( d == 1 ) {
					...
					while ( n < k ) {
						...
						if ( temp != ords[n] ) { err_printf ("Order mismatch 2, %lu != %lu for element ", temp, ords[n]);  _jac_print(a[n]);  exit (0); }
						n++;					 	
					}
					k--;
					if ( ! k) {
					...
					if ( (m%e[min]) != 0 ) { out_printf ("Problem, %lu not divisible by %lu\n", e[min], m);  exit (1); }
					jac_exp_ui (&b, a+k, m/e[min], c);
					_jac_mult (a[min], a[min], b, c[0]);
					for ( n = 0 ; n < k ; n++ ) {
						if ( n != min && e[n] ) {
							if ( (e[n]%e[min]) != 0 ) { out_printf ("Problem, %lu not divisible by %lu\n", e[min], m);  exit (1); }
							jac_exp_ui (&b, a+n, e[n]/e[min], c);
							_jac_mult (a[min], a[min], b, c[0]);
						}
					}
					dbg_printf ("Adjusted existing generator using relation, changed order from %lu to %lu.\n", ords[min], e[min]);
					ords[min] = e[min];
					temp = jac_pp_order_ui (a+min, p, h, c);
					if ( temp != ords[min] ) { err_printf ("Order mismatch 3, %lu != %lu for element ", temp, ords[min]);  _jac_print(a[min]);  exit (0); }
					goto repeat;
				}
			}
		} else {
			assert (k < 2*c->g);
			dbg_printf ("Added new independent generator %d with order %lu\n", k, ords[k]);
			k++;
		}
//printf ("%d generators: \n", k);
//for ( n = 0 ; n < k ; n++ ) { _jac_print(a[n]);  printf ("  order %ld\n", ords[n]); }
	}
	return k;
}
...
int jac_vector_logarithm (unsigned long e[], jac_t a[], unsigned long ords[], int k, jac_t beta[1], hc_poly hc[1])
{
	...
	// quick sanity check
	for ( i = 0 ; i < k ; i++ ) if ( ords[i] < 2 ) { out_printf ("Invalid order %lu specified in grp_vector_logarithm", ords[i]);  exit (0); }
...
//for ( i = 0 ; i < k ; i++ ) { out_printf ("(%d) ",ords[i]); _jac_print (a[i]); }
	...
	dbg_printf ("Vector DL in subgroup of order E = %lu\n", E);
	...
	if ( b >= stashsize ) {
		if ( stash ) {
			...
			stash = (jac_t *) mem_alloc (stashsize*sizeof(*stash));
		}
	}
	...
	for ( i = 1 ; i < b ; i++ ) {
		_jac_mult (g, g, a[0], hc[0]);
		for ( j = 0, m = ords[0] ; (i%m) == 0 ; m *= ords[++j] ) _jac_mult (g, g, a[j+1], hc[0]);
	...
	for ( s = 1, t = 0, u = 0 ; ! found ; s++ ) {
		...
		if ( t < ords[d]-1 && t+c >= ords[d] ) {
			_jac_mult (g, g, ht, hc[0]);
			t += ct;
			if ( t != ords[d]-1 ) { err_printf ("ct=%ld invalid t = %ld, c=%ld, ords[d] = %lu - didn't hit ords[d]-1 on center digit!", ct, t, c, ords[d]);  exit (0); }
		} else if ( t == ords[d]-1 ) {
			// carry
			_jac_mult (g, g, a[d], hc[0]);
			if ( d == k-1 ) goto done;
			t = 0;			// Note, must set t to 0 here, not reduce mod c
			_jac_mult (g, g, a[d+1], hc[0]);
			u++;
			// propogate carry as required
			for ( j = d+1, m = ords[d+1] ; (u%m) == 0 ; m *= ords[++j] ) {
				if ( j == k-1 ) goto done;
				_jac_mult (g, g, a[j+1], hc[0]);
			}
		} else {
			t += c;
			_jac_mult (g, g, h, hc[0]);
		}
	...
	if ( ! found ) {
		_jac_set (h, a[0]);
		for ( i = 1 ; i < k ; i++ ) {
			_jac_mult (h, h, a[i], hc[0]);
		}
		_jac_invert (h);
		_jac_mult (g, h, betainverse, hc[0]);
		n = smalljac_table_lookup (matches, &g);
		for ( i = 0 ; i < n ; i++ ) if ( _jac_cmp(stash[matches[i]], g) == 1 ) break;
		if ( i < n ) { found = 1;  m = matches[i]; } else found = 0;
