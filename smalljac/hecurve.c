...
int hecurve_sprint (char *s, ff_t u[HECURVE_GENUS+1], ff_t v[HECURVE_GENUS])
{
	...
	if ( _ff_nonzero(u[3]) ) {
		if ( _ff_one(u[3]) ) {
			return gmp_sprintf (s, "[x^3 + %ld*x^2 + %ld*x + %ld, %ld*x^2 + %ld*x + %ld]",
							_ff_get_ui(u[2]),_ff_get_ui(u[1]), _ff_get_ui(u[0]), _ff_get_ui(v[2]), _ff_get_ui(v[1]), _ff_get_ui(v[0]));
		} else {
			err_printf ("Warning, non-monic u in hecurve_sprint\n");
			return gmp_sprintf (s, "[%ld*x^2 + %ld*x + %ld, %ld*x + %ld]",
							_ff_get_ui(u[3]),_ff_get_ui(u[2]),_ff_get_ui(u[1]), _ff_get_ui(u[0]), _ff_get_ui(v[2]),_ff_get_ui(v[1]), _ff_get_ui(v[0]));
		}
	}
#endif
#if HECURVE_GENUS >= 2
	if ( _ff_nonzero(u[2]) ) {
#if HECURVE_GENUS == 3
		if ( _ff_nonzero(v[2]) ) gmp_err_printf ("Warning, deg v == deg u detected in hecurve_sprint, v[2] = %ld\n", _ff_get_ui(v[2]));
#endif
		if ( _ff_one(u[2]) ) {
			return gmp_sprintf (s, "[x^2 + %ld*x + %ld, %ld*x + %ld]",
							_ff_get_ui(u[1]), _ff_get_ui(u[0]), _ff_get_ui(v[1]), _ff_get_ui(v[0]));
		} else {
			err_printf ("Warning, non-monic u in hecurve_sprint\n");
			return gmp_sprintf (s, "[%ld*x^2 + %ld*x + %ld, %ld*x + %ld]",
							_ff_get_ui(u[2]),_ff_get_ui(u[1]), _ff_get_ui(u[0]), _ff_get_ui(v[1]), _ff_get_ui(v[0]));
		}
	} else if ( _ff_nonzero(u[1]) ) {
		if ( _ff_nonzero(v[1]) ) gmp_err_printf ("Warning, deg v == deg u = 1 detected in hecurve_sprint, v = %ldx+%ld\n", _ff_get_ui(v[1]),_ff_get_ui(v[0]));
		if ( _ff_one(u[1]) ) {
			return gmp_sprintf (s, "[x + %ld, %ld]", _ff_get_ui(u[0]), _ff_get_ui(v[0]));
		} else {
			err_printf ("Warning, non-monic u in hecurve_sprint\n");
			return gmp_sprintf (s, "[%ldx + %ld, %ld]", _ff_get_ui(u[1]), _ff_get_ui(u[0]), _ff_get_ui(v[0]));
		}
	} else if ( _ff_nonzero(u[0]) ) {
		if ( _ff_nonzero(v[1]) || _ff_nonzero(v[0]) ) gmp_err_printf ("Warning, deg v >= deg u = 0 detected in hecurve_sprint, v = %ldx+%ld\n", _ff_get_ui(v[1]),_ff_get_ui(v[0]));
		if ( _ff_one(u[0]) ) {
			return gmp_sprintf (s, "[1, 0]");
		} else {
			err_printf ("Warning, non-monic u in hecurve_sprint\n");
			return gmp_sprintf (s, "[%ld, 0]", _ff_get_ui(u[0]));
		}			
	} else {
		err_printf ("u is zero in hecurve_sprint!\n");  exit (0);
	}
#else
	if ( ! _ff_zero(u[1]) && ! _ff_one(u[1]) ) { printf ("non-monic u in hecurve print, lc=%lu(raw=%lu,one=%lu(\n", _ff_get_ui(u[1]),u[1],_ff_mont_R); exit(0); }
...
{
	ff_t t[2*HECURVE_GENUS-1], g[HECURVE_DEGREE+1];
	int d_u, d_v, d_t, d_g, d_f;

	d_f = ff_poly_degree (f, HECURVE_DEGREE);
	...
	if ( ! _ff_one(u[1]) ) { out_printf ("hecurve verification failed - non-affine point in genus 1, u[1]=%lu\n", _ff_get_ui(u[1])); return 0; }
	_ff_neg (x, u[0]);
	ff_poly_eval (&y, f, d_f, &x);
	ff_square (z, v[0]);
	if ( ! _ff_equal (y,z) ) {
	...
	if ( ! _ff_one(u[d_u]) ) {
		...
		gmp_printf ("d_u = %d, u[d_u] = %d (raw) %ld\n", d_u, u[d_u], _ff_get_ui(u[d_u]));
		return 0;
	}
	d_v = _deg_v(v);
	ff_poly_mult (t, &d_t, v, d_v, v, d_v);						// t = v^2
	ff_poly_sub (g, &d_g, f, d_f, t, d_t);							// g = f-v^2
	ff_poly_mod_small_inplace (g, d_g, u, d_u);	
	if ( ff_poly_degree(g,d_u-1) >= 0 ) {
		out_printf ("hecurve verification failed for element: \n    ");  hecurve_print (u,v);
		out_printf ("f-v^2 mod u = ");  ff_poly_print (g, d_g);
...
int hecurve_random_point (ff_t px[1], ff_t py[1], ff_t f[HECURVE_DEGREE+1])
{
	...
	do {
		...
		_ff_random(x);
///		dbg_printf ("Random x = %ld in F_%ld\n",_ff_get_ui(x), _ff_p);
		ff_poly_eval (&y, f, d_f, &x);
///		dbg_printf ("y^2 = f(%ld) = %ld mod %ld, computing square root\n",_ff_get_ui(x),_ff_get_ui(y), _ff_p);
	} while ( ! ff_sqrt (&z, &y) );
	if ( ui_randomb(1) ) ff_negate (z);	// flip square root 50% of the time
///	dbg_printf ("Found square root %ld\n", _ff_get_ui(z));
	_ff_set(px[0],x);
	_ff_set(py[0],z);
#if ! HECURVE_FAST
	if ( dbg_level >= 2 ) {
		ff_poly_sprint (buf, f, d_f);
		gmp_dbg_printf ("Random Point (%ld,%ld) on y^2 = %s over F_%ld\n",_ff_get_ui(px[0]), _ff_get_ui(py[0]), buf, _ff_p);
	}
#endif
	return 1;
}
...
void hecurve_random (ff_t u[HECURVE_GENUS+1], ff_t v[HECURVE_GENUS], ff_t f[HECURVE_DEGREE+1])
{
	...
	if ( ! hecurve_verify (u, v, f) ) {
		...
		gmp_err_printf ("P1 = (%ld,%ld)\n", _ff_get_ui(x1,0), _ff_get_ui(y1,1));
	}
#endif
#if ! HECURVE_FAST
	...
	if ( d_f == 5 ) {
		...
		// TODO: fix this!
		// resort to using divisors constructed from rational points for y^2=sextic case
		_hecurve_set_zero (u, v);
		if ( ! hecurve_random_point (&x1, &y1, f) ) { _hecurve_set_identity(u,v);  return; }
		do {
			if ( ! hecurve_random_point (&x2, &y2, f) ) { _hecurve_set_identity(u,v);  return; }
		} while ( d_f == 6 && _ff_equal(x1,x2) );		// for sextic case force 2 points into the divisor
		if ( _ff_equal(x1,x2) ) {
			hecurve_make_1 (u, v, x1, y1);
		} else {
	...
	// TODO: deal with sextic case where we must have degree 2 divisors
	if ( d_f == 6 ) { printf ("y^2=sextic curves not supported with HECURVE_RANDOM == 2\n"); abort(); }
	_hecurve_set_zero (u, v);
...
//printf ("alpha=%ldz+%ld\n", _ff_get_ui(alpha[1]), _ff_get_ui(alpha[0]));
			ff2_poly_eval_ff(beta, f, d_f, alpha);
...
//printf ("alpha=%ldz^2+%ldz+%ld\n", _ff_get_ui(alpha[2]), _ff_get_ui(alpha[1]), _ff_get_ui(alpha[0]));
			ff3_poly_eval_ff(beta, f, d_f, alpha);
...
#endif
#if ! HECURVE_FAST
	if ( dbg_level >= 2 ) { printf ("Random element: ");  hecurve_print (u, v); printf ("on curve (p=%ld) y^2 = ", _ff_p); ff_poly_print(f,d_f); }
#endif
#if HECURVE_VERIFY
	if ( ! hecurve_verify (u, v, f) ) {
		err_printf ("Verification failed in hecurve_random!\n");
#if HECURVE_GENUS == 3
		gmp_err_printf ("P1 = (%ld,%ld), P2 = (%ld,%ld), P3 = (%ld,%ld)\n",
				_ff_get_ui(x1), _ff_get_ui(y1), _ff_get_ui(x2), _ff_get_ui(y2), _ff_get_ui(x3), _ff_get_ui(y3));
#else
		gmp_err_printf ("P1 = (%ld,%ld), P2 = (%ld,%ld)\n", _ff_get_ui(x1), _ff_get_ui(y1), _ff_get_ui(x2), _ff_get_ui(y2));
#endif
		exit (0);
	}
#endif
...
{
	register ff_t s1, s2, s3, t1, t2, t3, t, w, z;
#if ! HECURVE_FAST
...
{
	ff_t d1[HECURVE_GENUS+1], e1[HECURVE_GENUS+1], e2[HECURVE_GENUS+1];
	ff_t d[HECURVE_GENUS], c1[HECURVE_GENUS], c2[HECURVE_GENUS];
	ff_t s1[FF_POLY_MAX_DEGREE+1], s2[FF_POLY_MAX_DEGREE+1], s3[FF_POLY_MAX_DEGREE+1];
	ff_t q[FF_POLY_MAX_DEGREE+1], r[FF_POLY_MAX_DEGREE+1], s[FF_POLY_MAX_DEGREE+1], t[FF_POLY_MAX_DEGREE+1];
	...
	d_u1 = _deg_u(u1);    d_u2 = _deg_u(u2);    d_v1 = _deg_v(v1);   d_v2 = _deg_v(v2);  d_f = ff_poly_degree (f, HECURVE_DEGREE);
	if ( _hecurve_is_identity(u1,v1) ) { _hecurve_set(u,v,u2,v2); return; }
	if ( _hecurve_is_identity(u2,v2) ) { _hecurve_set(u,v,u1,v1); return; }
//out_printf ("Cantor composition for curve y^2 = ");  ff_poly_print(f,d_f); printf ("mod %ld\n", _ff_p);
//out_printf ("    D1 := ");  hecurve_print (u1,v1);
//out_printf ("    D2 := ");  hecurve_print (u2,v2);

	_poly_gcdext (d1, e1, e2, u1, u2);
	_poly_add (t, v1, v2);
	_poly_gcdext (d, c1, c2, d1, t);
	_poly_mult(s1,c1,e1);  _poly_mult(s2,c1,e2);  _poly_set(s3,c2);
//printf ("d="); _poly_print(d);
//printf ("s1="); _poly_print(s1);
//printf ("s2="); _poly_print(s2);
//printf ("s3="); _poly_print(s3);
	_poly_mult (q, u1, u2);
//printf("u1*u2="); _poly_print(q);
	_poly_mult (t, d, d);
	_poly_div (s, r, q, t);			// s = u1u2/d^2		(s = u)
//printf ("u1*u2/d^2="); _poly_print(s);
	if ( d_r != -1 ) { err_printf ("Inexact division in Cantor's algorithm, r = ");  _poly_print(r);  exit (0); }
	_poly_mult (r, u1, v2);
	_poly_mult (t, r, s1);
//printf("s1*u1*v2="); _poly_print(t);
	_poly_mult (r, u2, v1);
	_poly_mult (q, r, s2);
//printf("s2*u2*v1="); _poly_print(q);
	_poly_addto (t, q);
	_poly_mult (r, v1, v2);
	_poly_addto (r, f);
	_poly_mult (q, r, s3);
//printf("s3*(v1*v2+f)="); _poly_print(q);
	_poly_addto (t, q);
	_poly_div (q, r, t, d);
	if ( d_r != -1 ) { err_printf ("Inexact division in Cantor's algorithm, r = ");  _poly_print(r);  exit (0); }
//printf ("v="); _poly_print(q);
	_poly_mod (t, q, s);			// t = (s1u1v2+s2u2v1+s3(v1v2+f))/d mod u
//printf("u="); _poly_print(s);
//printf("v="); _poly_print(t);
...
{
	register ff_t s, A, B, a, b, c, t0, t1, w1,w2,w3,w4;
	ff_t T0, T1;
	...
	_ff_addto (t1, f[1]);
	_ff_sub (T0, f[0], t1);
	if ( ! ff_sqrt(v,&T0) ) { /*dbg_printf ("unbits: sqrt(f0-f1+u0^2) = sqrt(%ld) failed\n", _ff_get_ui(t0,0)); */ return 0; }
	if ( (bits&0x1) != _ff_parity(v[0]) ) ff_negate(v[0]);
	return 1;
#endif
	
#if HECURVE_GENUS != 2
	err_printf ("Unsupported genus %d in hecurve_unbits\n"); abort();
#endif
	...
	if ( _ff_zero(a) ) {
		...
		_ff_square(T0,b);
		_ff_subfrom(T0,t1);
		if ( ! ff_sqrt(&T1,&T0) ) { /*dbg_printf ("unbits: sqrt(b^2-4ac) = sqrt(%ld) failed\n", _ff_get_ui(t1,0));*/  return 0; }
		_ff_add(t0,a,a);
		ff_invert(t0,t0);
		_ff_sub(s,T1,b);
	...
	if ( ! ff_sqrt(v,&T0) ) { /*dbg_printf ("unbits: sqrt(u[1]s0+f[0]) = sqrt(%ld) failed\n", _ff_get_ui(t0, 0));*/   return 0; }
	if ( _ff_zero(v[0]) ) {
		// use (14.8) to calculate v[1] when v[0] is zero - note that 14.8 is just v[1]^2 = s+A where A was computed above
		_ff_add(T0,s,A);
		if ( ! ff_sqrt(v+1,&T0) ) { /*dbg_printf ("unbits: sqrt(s0+f[2]-f[3]u[1]+u[1](2u[0]-u[1]^2) = sqrt(%ld) failed\n", _ff_get_ui(t0,0));*/  return 0; }
